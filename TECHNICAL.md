...
1. `update_master_if_needed(pairs, mode)`（pair-list作成直後）: `get_brand_new_drawing_pairs(pairs, mode)` で対象を求め、`relation='完全新規図面'` を付与して先行登録（エンティティ数はまだ不明）
2. `create_diff_zip()` 内（差分抽出時）: 同じ対象について `count_entities_in_dxf_file(main_file_info['temp_path'])` でエンティティ数を算出し追記。ファイル未アップロードの図番（`main_file_info` が無い）はスキップ（先行登録のみで終わる）
...
- **`one_sided`**（流用先が空白）: 比較対象の新図面がそもそも存在しないため差分比較は行わないが、「片側のみのペア」一覧に表示する。
- **`no_source_defined`**（流用先はあるが流用元が空白、2026-06 修正）: 流用元の参照が無い完全新規図面として扱う（方式A/Bの「孤立」パスと同じ意味合い）。以前は流用元空白も `one_sided` に分類していたため、「片側のみのペア」に表示され「完全新規図面」セクションに出てこない不具合があった（実データの `引当前後リスト_ME25-9606-0` の `DE3527-556-01B` で確認）。
...
`render_pair_list()`（Step3表示）と `update_master_if_needed()` / `create_diff_zip()`（台帳登録）の両方から呼ばれる共通ロジック。「変更していない図面」対象の図番集合（`compute_unchanged_drawings`）と、それを除外した完全新規図面ペアのリスト（`get_brand_new_drawing_pairs`）を返す。2か所で同じ集合を使うことを保証するため、render_pair_list() 内にあった同等のインラインロジックをこの共通関数に統合した（2026-06）。
