...
| ⚠️ 図面ファイルがない図番（**Type C のみ**） | `missing_source` / `missing_target` / `missing_both` を統合（2026-06変更） | 旧来の3セクション（流用元図番の図面がない図面／流用先のDXFファイルが未アップロード／流用元・流用先ともに未アップロード）をType Cのみ1セクションに統合。ステータス列は「流用元 図面ファイルなし」「流用先 図面ファイルなし」「流用元・先 図面ファイルなし」のいずれか。Type Cはrelationが常に'ペアリスト'でRevUpという概念が無いため、RevUp注記は不要 |
| ➖ 流用先がない流用元図面 | `one_sided`（pair_list のみ） | 旧名「片側のみのペア」（2026-06改名）。ペアリストで流用元は記載しているが流用先が空白の行 |
...
- **`one_sided`**（流用先が空白）: 比較対象の新図面がそもそも存在しないため差分比較は行わないが、「流用先がない流用元図面」一覧に表示する（2026-06 改名。旧名「片側のみのペア」）。
...
台帳が読み込まれている場合のみ `update_parent_child_master()` を呼ぶラッパー。`complete` ペアに加え、`get_brand_new_drawing_pairs(pairs, mode)` で求めた完全新規図面（`relation='完全新規図面'` を付与）も登録対象に含める（2026-06 追加。エンティティ数はこの時点では未確定）。
...
#### `compute_unchanged_drawings(all_pairs, mode)` / `get_brand_new_drawing_pairs(all_pairs, mode)`
...
- 流用元・流用先それぞれの未アップロード一覧を別々に表示する（同一図番なら両方に登場し得る）
...
（`app.py` の `create_diff_zip()`）はこの値を `entity_counts = {'added_entities': count, 'total_entities': count}` として `update_parent_child_master()` に渡す。
...
*最終更新: 2026-06-24（(1) Type C の「変更していない図面」を、流用先のDXFファイルが実在する（`main_file_info` がある）identical図番のみに再度限定（`compute_unchanged_drawings`）。ファイル未アップロードの identical 図番を含めると、Summaryシートの流用先図面総数(a)（実ファイルがある図番のみで算出）と Step3 の `差分抽出が可能なペア+完全新規図面+変更していない図面` の合計が一致しない不整合が生じるため（実データで a=54 に対し合計56 と確認）。これに伴い、ファイル未アップロードの identical 図番も `_show_missing_drawings()`（Step2-2 の未アップロード警告。流用元/流用先が同一図番の行も対象に含めるよう修正）で警告表示されるようにした。(2) Type C のみ、Step3の「流用元図番の図面がない図面」「流用先のDXFファイルが未アップロード」「流用元・流用先ともに未アップロード」の3セクションを「図面ファイルがない図番」1セクションに統合（ステータス列で「流用元/流用先/流用元・先 図面ファイルなし」を区別）。Type A/Bは元のまま変更なし）*
