...
7. [model/extract_labels.py 詳解](#7-modelextract_labelspy-詳解)
8. [model/compare_dxf.py 詳解](#8-modelcompare_dxfpy-詳解)
9. [model/label_diff.py 詳解](#9-modellabel_diffpy-詳解)
10. [model/common_utils.py 詳解](#10-modelcommon_utilspy-詳解)
11. [図面番号フォーマット仕様](#11-図面番号フォーマット仕様)
12. [出力ファイル仕様](#12-出力ファイル仕様)
13. [プロジェクト間 model/utils 同期戦略](#13-プロジェクト間-modelutils-同期戦略)
...
├── sync_utils.py             # DXF-visual-diff との model/utils 同期スクリプト
├── 図面管理台帳.xlsx          # サンプル図面管理台帳（開発・テスト用）
├── model/                    # 2026-07-15: utils/ からリネーム（3層構造の明確化）
│   ├── __init__.py
...
│   └── common_utils.py       # 共通ユーティリティ（ファイル保存・機器符号フィルタ等）
├── tests/
│   ├── unit/                 # モデル層のユニットテスト（test_pairing.py / test_master_ledger.py /
│   │                         #   test_compare_dxf.py / test_label_diff.py / test_diff_export.py）
...
直接依存しないロジックを `model/`（2026-07-15以前は`utils/`）配下のモデル層へ分離した
（dev-workflow スキルの3層構造に対応）。**動作は変更していない**——コードの移動と、
`st.error()` 直接呼び出しを `on_error` コールバック／戻り値タプルに置き換える最小限の変更のみ。
...
| Model | `model/pairing.py` | ペア生成・Step3表示分類ロジック（`build_pairs*`、`compute_unchanged_drawings`、`get_brand_new_drawing_pairs` 等） |
| Model | `model/master_ledger.py` | 図面管理台帳の読み込み・更新・Excel出力（`load_parent_child_master`、`update_parent_child_master`、`create_empty_master_df`、`save_master_to_bytes`） |
| Model | `model/diff_export.py` | `create_diff_zip()`（差分DXF・ラベル差分Excel・台帳更新を1つのZIPにまとめる） |
...
### モデル層 `model/master_ledger.py`（2026-06 新設）
...
**複数フォルダは1つずつアップロードする（2026-08 追加）**: 複数のフォルダをまとめて
1回のドラッグ&ドロップで渡すと、対象外拡張子のファイルが混在している場合にブラウザ側の
フォルダ再帰読み込みが壊れ、一部のファイルが読み込まれないことが実データで確認されている
（`type=` パラメータとフォルダ再帰読み込みの相互作用による不具合。詳細は Section 11
「ファイル名フィルタとしての利用」参照）。複数フォルダを扱う場合は、フォルダを1つずつ
順番にドラッグ&ドロップ→「ファイルを読み込む」をクリック、を繰り返すことを各Step2の
キャプションで案内している。

**受理されたDXFファイルの一覧表示（`render_accepted_files_table()`、2026-08 追加）**:
`st.file_uploader` 自体のファイル一覧（ドロップ直後のプレビュー）は、選択されたファイル
すべて（図番フォーマットに一致しないものも含む）を表示する仕様であり、これを絞り込む/
非表示にする公開APIが無い。そのため「ファイルを読み込む」実行後、実際に受理された
（＝図番フォーマットに一致しファイル情報の抽出に成功した）DXFファイルだけを一覧表示する
折りたたみ式テーブル（図番・ファイル名、流用先系のモードは抽出した流用元図番も表示）を
各Step2セクションに追加し、こちらを正式な確認手段として案内している。

**アップロード欄ラベルの統一（2026-08 追加）**: 4箇所の `st.file_uploader` のラベルを
すべて「DXFファイル」で始まる表記に統一した。Type A（`DXFファイル（複数可・フォルダ可・
複数回可）`）・Type C（同左）は共通、Type B（auto）の2箇所のみ役割を示す修飾を付ける
（`DXFファイル（流用元/旧、複数可・フォルダ可・複数回可）` / `DXFファイル（流用先/新、
複数可・フォルダ可・複数回可）`）。以前は Type B のみ「流用元（旧）DXFファイルを
アップロードしてください（...）」という異なる文体だった。

**「直近のDXFファイル読み込み」→「直近の入力ファイル読み込み」（`render_upload_status()`、
2026-08 変更）**: `type=` を指定しない file_uploader（前述の重大な修正）のため、入力欄には
DXF以外のファイル（xlsx等）が混ざり得る。「DXFファイル読み込み」という表現は入力全体が
DXFであるかのように誤解を招くため、「入力ファイル読み込み: 入力{total_input}件中、
DXFファイルとして{processed}件を読み込みました」という表現に変更し、入力総数と実際に
DXFとして受理された件数を分けて示す。`process_all_uploaded_files()` の返す summary dict
（`st.session_state.*_upload_summary`）に `total_input`（フィルタ前の生アップロード件数）
を追加して実現。`アップロードできなかった{label}ファイル`（失敗一覧の見出し）は
引き続き `label`（流用元/流用先/DXF）を使う——これは図番フォーマットには一致したが
抽出に失敗したファイルのみを指すため、`DXF`表記のままで正確。
...
- オプション設定（移動しただけのラベルを差分から除外・色だけが異なる図形は変更なし扱い・機器符号妥当性チェック・座標マージン・未変更ラベルプレフィックス・レイヤー色）を確認
- 「差分抽出開始」ボタンをクリック
...
### 「ファイル名を確定」→「ZIPでダウンロード」の確定フロー仕様（2026-08 確定）

**なぜ確定ステップが必要か**: `st.download_button()` はクリック時にサーバーへ
再接続してファイル名を再計算しない——直前の rerun でレンダリングされた内容を
ブラウザがそのまま使ってダウンロードするだけの仕組みのため、`zip_basename_input`
を編集した直後（Enter/blurで確定する前）に「ZIPでダウンロード」を押すと、**編集前の
古いファイル名**（既定の `..._01` 等）でダウンロードされてしまう不具合があった
（実データで確認。ユーザー報告: 末尾の `_01` を消して確定しても消える前の名前で
ダウンロードされる）。

**仕様（確定版）:**

1. **Step 5 に新しく入った時点では、ファイル名がデフォルトのままであっても常に
   未確定の状態から始まる。** 表示されるのは「ファイル名を確定」ボタンのみで、
   「ZIPでダウンロード」は表示されない。
2. **確定はボタンクリックでのみ行う。** テキスト入力欄でのEnter/blurは確定として
   扱わない——ユーザーが確定したつもりのない状態で古い/新しい値が紛れる余地を
   無くすため、確定手段を「ファイル名を確定」ボタン1つに一本化する（2026-08
   ユーザー確認。過去に一度 `on_change` によるEnter自動確定を試みたが、
   「Enterでは確定しない、常にボタンクリックを要求する」という明示的な要望を
   受けて撤回した）。
3. **「ファイル名を確定」を押すと、その時点の入力欄の内容で確定し、
   「ZIPでダウンロード」に切り替わる。**
4. **確定後にファイル名を再度編集すると、再び未確定の状態に戻る。** ダウンロード
   するには改めて「ファイル名を確定」を押す必要がある。

**実装**: `st.session_state.zip_basename_confirmed` を「確定済みファイル名」として
新設し、`download_button` の `file_name` にはこの確定済みの値だけを渡す。
**未確定を表す番人として `None` を使う**（Step 5 に新しく入った時点、および
デフォルト値が変わって自動追従する時点で `None` にリセットする）。表示切り替えの
判定は次の通り:
...
`zip_basename_confirmed is None` の項が「Step 5 に新しく入った時点では常に未確定」を
保証し、`input != confirmed` の項が「確定後に再編集したら未確定に戻る」を保証する。

「ファイル名を確定」ボタン（通常の `st.button`）はクリックのたびにその時点の
全ウィジェットの値（未確定の編集中テキストも含む）を携えて rerun するため、
`download_button` と違い staleness が起きない——この非対称性を利用し、確定ボタンを
経由させることで、確定の瞬間に必ず最新の入力値を `zip_basename_confirmed` に反映させる。

**ボタンの表示切り替え（streamlitスキル§11「誤操作のリスクがある場合は非表示切り替え」
参照）**: `needs_confirmation` の間は「ファイル名を確定」のみを青色
（`type="primary"`）で表示し、「ZIPでダウンロード」は非表示にする。確定すると逆に
「ファイル名を確定」を非表示にし、「ZIPでダウンロード」を青色で表示する。単なる
色分けではなく非表示にしているのは、押すと実際に誤ったファイル名・未確定のまま
ダウンロードされてしまう（単なる二度手間ではない）ため、押せる余地自体を無くすのが
安全という判断（一般的な「完了後も白ボタンとして残す」パターンとは異なる。詳細は
streamlit スキル参照）。

**動作確認（Playwright、5シナリオ全て確認済み）:**

| シナリオ | 期待される表示 |
|---|---|
| Step 5 初回表示（デフォルト名のまま、未編集） | 「ファイル名を確定」のみ表示 |
| 「ファイル名を確定」をデフォルト名のまま押す | 「ZIPでダウンロード」に切り替わり、デフォルト名でダウンロードできる |
| 編集してEnterを押しただけ（ボタンは押さない） | 「ファイル名を確定」のまま（Enterでは確定しない） |
| 編集後に「ファイル名を確定」を押す | 編集後の名前で確定し「ZIPでダウンロード」に切り替わる。実際にその名前でダウンロードされる |
| 確定済みの状態からファイル名を再編集 | 再び「ファイル名を確定」のみの表示に戻る |
...
`model/master_ledger.py` / `model/diff_export.py` / `model/pairing.py`（モデル層）に
...
| `zip_basename_confirmed` | str | 「ファイル名を確定」ボタンで確定済みのファイル名。`download_button` の `file_name` にはこの値のみを使う（`download_button` のクリック時staleness対策、2026-08 追加。下記参照） |
...
#### `load_parent_child_master(uploaded_file)`（2026-06: `model/master_ledger.py` へ移動。2026-07: シート自動検出に変更）
...
#### `update_parent_child_master(master_df, new_pairs)`（2026-06: `model/master_ledger.py` へ移動）
...
#### `compute_unchanged_drawings(all_pairs, mode)` / `get_brand_new_drawing_pairs(all_pairs, mode)`（2026-06: 実体を `model/pairing.py` へ移動・引数化）
...
#### `extract_base_drawing_number(drawing_number)`（`model/pairing.py`）
...
#### RevUpペアリングのロジック (`find_revup_pairs`、`model/pairing.py`）
...
**対策**: `model/pairing.py` の `STATUS_DISPLAY_PRIORITY`（`complete` > `missing_source` > `missing_target` > `missing_both` > `one_sided` > `identical` > `no_source_defined`）と `primary_status_by_drawing(pairs)` で、main_drawing ごとに「最も優先度の高いステータス」を1つだけ決定する。`render_pair_list()` はこれを使い、各ステータス別の表示行（missing_source/missing_target/missing_both/identical/no_source_defined）を、その図面の優先ステータスと一致する行のみに絞り込む（`complete` と判定された図面は他のどのセクションにも現れない）。

- `差分抽出が可能なペア`（complete）の表は実際に生成される全ペアをそのまま表示する。タイトルの「：N件」は**表の行数（＝`len(complete_pairs)`、実際に差分抽出が実行される比較回数）と一致する**（2026-08 変更。以前は main_drawing のユニーク数を使っており、同一図面が複数の流用元と比較される場合——RevUp と流用の双方で complete になる等——に表の行数より少ない件数が表示され、実データで「テーブルは5行なのに4件と表示される」とユーザーが誤解する事象が発生したため、テーブル行数優先に変更。Summary シートの「差分抽出ペア数」（`save_master_to_bytes` 内 `pair_count`、元から `len(complete_pairs)`）と表記が揃った）。
  - **トレードオフ**: 同一図面が複数の関係（RevUp と流用の双方）で `complete` になる場合、本セクションの件数と他セクション（`missing_source`・`変更していない図面`・`完全新規図面`）の件数を合計しても、下記「常に一致する」という不変条件は厳密には成り立たなくなる（複数関係を持つ図面が実際の関係数ぶん重複計上されるため）。ユーザー確認の上でこの表示上のトレードオフを受け入れている。
- `missing_source` / `missing_target` / `missing_both` / `no_source_defined` のタイトルの「：N件」は引き続き main_drawing のユニーク数のまま（変更なし）。
...
- Type C（`pair_list`）: 排他化済み `identical_pairs`（`identical` が優先ステータスの図面のみ）のうち、**流用先のDXFファイルが実在する（`main_file_info` がある）図番のみ**を対象にする（2026-06 再修正）。
  ...
  **経緯（2回の修正を経て現在の仕様に至った）**:
  ...
  この結果、`差分抽出が可能なペア(20、unique)+完全新規図面(1)+変更していない図面(33、ファイル実在のみ)=54=流用先図面総数(a)` が（重複関係を持つ図面が存在しない場合は）一致する。**2026-08 の変更後は「差分抽出が可能なペア」がユニーク数ではなく表の行数になったため、複数関係を持つ図面が存在するデータではこの等式は厳密には成り立たなくなる**（上記トレードオフ参照）。

**検証方法**: 実データ（`sample-dxf/ME24-1001-0/`、流用先232件、重複関係の図面なし）で `差分抽出が可能なペア(73, ユニーク＝表の行数と一致)` + `流用元図番の図面がない図面(154)` + `変更していない図面(5)` + `完全新規図面(0)` = 232 と一致することを確認。回帰テスト: `tests/unit/test_pairing.py` の `test_primary_status_prefers_complete_over_*`（`primary_status_by_drawing()` 自体の挙動は変更していないため、これらのテストは影響を受けない）。
...
## 7. model/extract_labels.py 詳解

DXFファイルからテキストラベル・図番・タイトルを抽出するコアモジュール。

> **DXF-extract-labels との共通化（2026-05）**: `DXF-diff-manager/model/extract_labels.py` と `DXF-extract-labels/model/extract_labels.py` は同一ファイルに統一されている。変更は必ず両プロジェクトのファイルを同一内容に保つこと（`diff` コマンドで確認）。適応的設定読み込みパターン（Section 7.1）により、`config.py` の有無にかかわらず両環境で動作する。
...
このパターンにより、同一ファイルが `DXF-diff-manager` と `DXF-visual-diff` の両方で動作する（詳細は [Section 13](#13-プロジェクト間-modelutils-同期戦略) 参照）。
...
生の文字列ではなく `clean_mtext_format_codes()`（`model/extract_labels.py`。diff_labels.xlsx
...
**`ignore_color`（座標・形状が一致し color だけ異なる場合の扱い、2026-07 追加）**:

`SignatureGenerator.__init__(transformer, debug=False, ignore_color=False)` の
`ignore_color=True` で、署名の `_add_important_attributes()` から `color` を除外する
（`important_attrs = ['color', 'height', 'radius', 'start_angle', 'end_angle']` から
`color` だけを取り除く）。`height`/`radius`/`start_angle`/`end_angle` は幾何形状そのものを
表す属性なので対象外——`color` のみが「座標・形状に影響しない純粋な描画スタイル属性」で
あるため、これだけを個別に除外可能にしている。`lineweight` は元々署名に含まれていない
（`_add_important_attributes`/`_add_geometry_details` のどちらにも無い）ため、この機能を
待たずとも lineweight 差だけでは元々差分と判定されない。

Step4「オプション設定」の**色だけが異なる図形は変更なし扱いにする**
（`ignore_color_only_changes`）用。`compare_dxf_files_and_generate_dxf()` /
`count_entities_in_dxf_file()` の両方に同名引数があり、`SignatureGenerator` の
`ignore_color` にそのまま渡す。両関数で揃える理由: 完全新規図面の Total Entities は
`count_entities_in_dxf_file()` で算出するが、これが `compare_dxf_files_and_generate_dxf()`
と異なる `ignore_color` 設定で数えると、台帳内で Deleted/Added/Unchanged Entities と
Total Entities の「同一エンティティ」の定義が食い違ってしまう。

背景: 実データ `EE2505-633-43E_vs_43D` で、座標・形状（LINE の start/end、CIRCLE の
center/radius）が完全一致するのに `color` だけが異なる（かつ多くは `lineweight` も
異なる）エンティティが334件、DELETED＋ADDED として検出されていた（R1ラベル近傍の
抵抗シンボルLINE・F8ラベル近傍のヒューズCIRCLE等で確認）。内訳を集計すると
`lineweight 25→50・color 7→4`（231件）／`50→25・4→2`（73件）／`25→25・7→2`（30件）と
パターン化されており、偶然のノイズではなく改訂マーキング等による**意図的な色分け変更**
と考えられる。これは MTEXT フォーマットコードや off/frozen レイヤーのような「本来一致
すべきなのに誤って不一致になる」不具合とは性質が異なり、**色の違いは実際に見た目が変わる
本物の変更**なので、無条件に UNCHANGED 化する（デフォルト動作にする）のは不適切——
色分けマーキング自体を検知したいケースもあるため、ユーザーに意図を確認したうえで
**オプトインの選択肢**として実装した。回帰テスト: `tests/unit/test_compare_dxf.py`
（`test_color_only_difference_detected_by_default` /
`test_color_only_difference_treated_as_unchanged_when_ignored` /
`test_ignore_color_does_not_hide_genuine_position_changes` /
`test_ignore_color_only_changes_end_to_end_via_compare_dxf_files` /
`test_count_entities_in_dxf_file_respects_ignore_color_only_changes`）。

### 8.6 `compare_dxf_files_and_generate_dxf(file_a, file_b, ...)`

メインの差分比較関数。

**file_a=旧図面、file_b=新図面の順で渡すこと**（標準的な diff の慣習: 旧→新）。
`file_a` のみに存在するエンティティが `DELETED`、`file_b` のみに存在するエンティティが
`ADDED` になる。呼び出し元がこの順序を逆にすると ADDED/DELETED レイヤーの内容が入れ替わる
（2026-07 に実際に発生した不具合。以前の本ドキュメントの記述自体が file_a=新・file_b=旧、かつ
「Aにのみ存在＝追加」という誤った対応を説明しており、`model/diff_export.py` の呼び出し順序が
これに合わせて逆になっていた。実データ `EE4144-613-49D_vs_EE4144-613-49C` で、ADDED レイヤーに
旧図面自身のテキスト・DELETED レイヤーに新図面自身のテキストと新規追加された改訂メモが
混入する形で発覚。回帰テスト: `tests/unit/test_compare_dxf.py` の
`test_file_a_only_is_deleted_file_b_only_is_added`、`tests/unit/test_diff_export.py`）。

```python
def compare_dxf_files_and_generate_dxf(
    file_a: str,            # 旧図面（流用元図番）のパス
    file_b: str,            # 新図面（図番）のパス
    ...
    ignore_color_only_changes: bool = False,  # 座標・形状一致・color差のみをUNCHANGED扱い（8.5節）
    debug: bool = False
) -> dict:
    """
    Returns: {
        'deleted_entities': int,   # Aにのみ存在（旧のみ＝削除）
        'added_entities': int,     # Bにのみ存在（新のみ＝追加）
...
（`model/diff_export.py` の `create_diff_zip()`）はこの値を `entity_counts = {'added_entities': count, 'total_entities': count}` として `update_parent_child_master()` に渡す。
...
（クラス定義は本関数の直前）は呼び出し元（`model/diff_export.py` の `create_diff_zip()`）が
...
### 9.2a `reclassify_moved_labels(change_rows, unchanged_entries)`（2026-07 追加）

Step4「オプション設定」の**移動しただけのラベルを差分から除外**（`ignore_moved_labels`）用の後処理。`find_label_change_pairs()` は座標単位でしか突き合わせないため、回路ブロックがそのまま別の座標へ丸ごと移動すると、移動元は「削除」・移動先は「追加」として検出される（結線・ラベル自体は変わっていないのに diff_labels.xlsx が大量の偽差分を報告する。実データ `EE6606-613-49A_vs_EE4144-613-49D` で確認: 変更行146→42、削除88→36・追加54→2、未変更386→438）。

```python
def reclassify_moved_labels(change_rows, unchanged_entries):
    """
    change_rows のうち Old Label のみ（削除）／New Label のみ（追加）の行を
    ラベル文字列でグループ化し、削除件数・追加件数が一致する分だけ
    unchanged_entries に振り替える（座標は新図面側を採用）。

    対象外:
      - Old/New 両方が存在する行（同一座標での名称変更）には影響しない
      - ラベルに「☆」を含む行（注記・特記事項として使われることが多いため、
        件数が一致していても常に変更候補として残す）
    """
```

**対応関係の非一意性**: 同一ラベルが複数箇所で移動した場合、どの旧座標とどの新座標が対応するかは一意に決まらない（同じラベル文字列は区別できないため）。座標順にソートした上で先頭から機械的に対応付ける。対応関係自体に意味はなく、あくまで「件数が一致した分だけ変更なしとして扱う」ためのものである。

**リスク（呼び出し元・UIヘルプで明示）**: 座標を見ず件数だけで判定するため、たまたま同じラベル名の部品が「別の場所で削除」され「別の無関係な場所に同名の部品が新規追加」された場合も区別できず「移動」とみなされる。デフォルトは無効（オプトイン）。

回帰テスト: `tests/unit/test_label_diff.py`。
...
**unchanged_labels.xlsx のシート順も同じABC順に揃える（2026-07 追加）**: diff_labels.xlsx 側だけソートすると、同一バッチから出力される2ファイル間でシート順が食い違い、両ファイルを突き合わせて確認する際に混乱する（dev-workflow スキルの選択肢組み合わせ表で「diff_labels ソート × unchanged_labels」を要確認と判定し、実際に順序不一致を確認したうえでユーザーに揃えるかどうかを確認して対応）。`build_unchanged_labels_workbook()` を呼ぶ直前に `unchanged_label_sheets` を `sheet_name`（＝図番）で直接ソートする（`summary_data` のような対になるリストが無いため、`diff_label_sheets` のようなインデックスベースの対応付けは不要で、単純にリスト自体をソートすればよい）。
...
**`handle_error()` の削除（2026-07 リファクタ）**: 以前は `import streamlit as st` して `st.error()` を呼ぶ `handle_error(e, show_traceback=True)` がこのファイルにあったが、Model層（streamlit非依存が方針）への層違反であり、呼び出し元も `app.py` の1箇所（`create_diff_zip()` 実行時の例外ハンドラ）のみだった。呼び出し元にインライン化し、本ファイルからは削除・`import traceback` も不要になった。
...
2. `DXF-diff-manager/model/extract_labels.py` → フォールバック `ExtractionConfig` クラス内の同名属性
...
**⚠️ `st.file_uploader()` の `type=` パラメータは指定しない（2026-08 重大な修正）**:
当初 `st.file_uploader(type=ui_config.DXF_FILE_TYPES)` としていたが、実データで
「フォルダをドラッグ&ドロップしてもRevUpペアが一部検出されない」不具合が発生した。
調査の結果、`type=` パラメータは単純な「アップロード後の拡張子フィルタ」ではなく、
**ブラウザ側のフォルダ再帰読み込み処理自体（react-dropzoneの内部実装）にも
`accept` 制限として作用し、対象外拡張子のファイル（`diff_labels.xlsx` 等）が混在する
フォルダをドロップすると、再帰読み込み自体が壊れて一部ファイルが丸ごと失われる**ことが
判明した。`.dxf` のみで構成されたフォルダなら正常動作し、xlsx等が混ざったフォルダ
（単一・複数どちらも）でのみ発生することが決め手だった。

対策として `app.py` の4箇所のDXF `file_uploader`（auto方式の流用元・流用先、
pair_list方式の一括、all_in_one方式）すべてから `type=` パラメータを削除し、
`is_drawing_number_filename()` によるサーバー側フィルタのみに一本化した
（拡張子を問わず全ファイルをブラウザから受け取り、Python側でファイル名フォーマットに
よって絞り込む）。台帳アップロード欄（`type=ui_config.MASTER_FILE_TYPES`）は
xlsx単体でフォルダ混在の懸念がないため変更していない。

**この不具合はPlaywrightの `set_input_files()` では再現できなかった**——同APIは
ブラウザの実際のフォルダ再帰読み込みコードを経由せず、ファイルのflat listを直接
input要素に注入するため、react-dropzoneの`accept`検証ロジックに起因するこの種の
不具合は原理的に再現不可能。実ブラウザでの手動確認によってのみ発見された（詳細は
streamlitスキル§3「フォルダD&D＋ファイル名フォーマットフィルタ」の重大な注意点を参照）。
...
## 13. プロジェクト間 model/utils 同期戦略

### 13.1 背景

`model/extract_labels.py` は複数のプロジェクトで共有されており、各プロジェクトの設定ファイル構成が異なる。
...
**同期方向**:
```
DXF-diff-manager/model/ → (sync_utils.py) → DXF-visual-diff/utils/
```
...
2. `DXF-diff-manager/model/extract_labels.py` 内のフォールバック `ExtractionConfig` クラスも**同じ値に**更新
...
完全新規図面数÷図面総数）を追加。
(5) Step5のZIPファイル名編集で、`st.download_button`がクリック時にファイル名を
再計算しない（直前renderの内容をブラウザがそのまま使う）仕様のため、テキスト編集後
Enter/blurせず直接ダウンロードすると編集前の古い名前（既定の`..._01`等）でダウンロード
されるユーザー報告バグを修正。「ファイル名を確定」ボタン（通常のst.button、編集中の
値も携えてrerunする）を経由させ、`zip_basename_confirmed`（確定済みの値のみ）を
download_buttonのfile_nameに使うよう変更。未確定中は確定ボタンのみ青色表示・
download_buttonは非表示、確定後は逆に切り替える（streamlitスキル§11の新パターン
「誤操作のリスクがある場合は非表示切り替え」を適用）。
(6) Step2のDXF `file_uploader`（4箇所）から `type=` パラメータを削除。実データで
「フォルダをドラッグ&ドロップしてもRevUpペアが一部検出されない」不具合が発生し調査した
結果、`type=` パラメータがブラウザ側のフォルダ再帰読み込み処理自体にも`accept`制限として
作用し、対象外拡張子ファイル（xlsx等）が混在するフォルダのドロップで再帰読み込みが壊れる
ことが判明（`.dxf`のみのフォルダなら正常動作）。サーバー側の`is_drawing_number_filename()`
フィルタのみに一本化して解消。Playwrightの`set_input_files()`では再現不可能な、実ブラウザ
でのみ確認できた不具合だった。詳細はSection 11「ファイル名フィルタとしての利用」参照。
(7) Step3「差分抽出が可能なペア：N件」の件数を、main_drawingのユニーク数から
`len(complete_pairs)`（表の行数＝実際の比較回数）に変更。同一図面が複数の関係
（RevUpと流用の双方）でcompleteになる場合に「テーブルは5行なのに4件と表示される」と
ユーザーが誤解する事象が実データで発生したため。Summaryシートの「差分抽出ペア数」と
同じ集計方法に統一。トレードオフとして、他セクション（未アップロード・変更していない
図面・完全新規図面）との合計が流用先総数と厳密には一致しなくなる場合があることを
ユーザー確認の上で許容。
(8) Step2の4セクション全てのキャプションに「複数のフォルダを読み込む場合は1つずつ
順番にアップロードしてください」を追記（(6)の不具合修正後も、複数フォルダ同時ドロップは
念のため避けるよう案内）。`st.file_uploader`自体のファイル一覧プレビューは対象外拡張子の
ファイルも含めて表示され絞り込む公開APIが無いため、代わりに「ファイルを読み込む」実行後に
実際に受理されたDXFファイルだけの一覧を折りたたみテーブルで表示する
`render_accepted_files_table()` を新設し、4箇所の単純な「N件」表示を置き換えた。
(9) 4箇所の `st.file_uploader` ラベルを「DXFファイル」始まりに統一（Type Bのみ
「流用元(旧)DXFファイルをアップロードしてください(...)」という異なる文体だったのを
「DXFファイル（流用元/旧、...）」に変更）。
(10) `render_upload_status()` の「直近のDXFファイル読み込み」を「直近の入力ファイル
読み込み：入力{total_input}件中、DXFファイルとして{processed}件を読み込みました」に変更。
(6)の`type=`除去により入力欄にDXF以外のファイルが混ざり得るため、「DXFファイル読み込み」
という表現が入力全体を指すかのように誤解を招くのを避けるため。
(11) Step5のZIPファイル名確定フローの仕様を確定させた。試行錯誤の経緯:
(a) `zip_basename_input`の`on_change`コールバックでEnter/blur時に自動確定する版を
一度実装したが、「確定はボタンクリックでのみ行うべき。Step 5に新しく入った時点では
デフォルト名のままでも常にボタンから始まるべき」という明示的な指摘を受けて撤回。
(b) `zip_basename_confirmed`の初期値を`None`（未確定の番人）にし、Step 5に新しく
入った時点・デフォルト値が自動追従する時点で常に`None`にリセットするよう修正
（以前は`confirmed`を`input`と同時に初期化していたため、未編集時は最初から
「ZIPでダウンロード」が表示されてしまっていた）。`on_change`は削除し、確定手段を
「ファイル名を確定」ボタンのクリックのみに一本化した。5パターン
（初回表示/デフォルト名で確定/編集してEnterのみ=確定されない/編集後にボタンで確定/
確定後の再編集で未確定に戻る）をPlaywrightで検証し、いずれも仕様通りであることを
確認。詳細な仕様はSection 4「『ファイル名を確定』→『ZIPでダウンロード』の確定フロー
仕様」参照）
(12) Step3の図面ペア・リスト表示（4箇所: 差分抽出が可能なペア/図面ファイルがない図番
[Type C]/流用元図番の図面がない図面[Type A/B]/完全新規図面）とStep4の差分抽出結果
テーブルが、いずれも「流用先（新）」でABC順になっていない不具合を修正。
`build_pairs()`（`model/pairing.py`）内部の処理順（RevUpペアがPythonの`set`走査順、
流用ペアがアップロード/辞書挿入順）がそのまま画面表示に漏れていたことが原因。
`complete_pairs`/`missing_file_pairs`/`missing_pairs`/`no_source_pairs`/`results`
自体は変更せず（`complete_pairs`はStep4処理・進捗バーにそのまま使われるため）、
各表示直前で `sorted(list, key=lambda p: p['main_drawing'] or '')` を適用する
表示専用の変更。Playwrightのスクリーンショット比較で両テーブルともABC順表示を確認。
(13) Drawing List更新（`update_drawing_list()`）実行時にコンソールへ出力されていた
`FutureWarning: The behavior of DataFrame concatenation with empty or all-NA
entries...` を修正。空の Drawing List（`create_empty_drawing_list_df()`、0行）に
新規レコードを `pd.concat()` で連結する処理が、pandas の将来のdtype決定変更に
関する警告を発生させていた。`update_parent_child_master()`（Diff List側）が既に
採用している「1行ずつ `updated_df.loc[len(updated_df)] = record` で追加」方式に
統一して解消（表示・出力内容は不変、警告解消のみ）。全97件pass）*
...
（`from utils.compare_dxf import`等）になっていたため、他ファイルの相対import方式に統一
（`from .compare_dxf import`等）。`sync_utils.py`（DXF-visual-diffとの同期スクリプト。
DXF-visual-diff側は`utils/`のまま未変更）のパス参照を、プロジェクトごとに`model`/`utils`
を切り替える`utils_subdir()`ヘルパー経由に変更。本ファイル内の現行アーキテクチャの
パス表記（目次・アンカー含む）も`model/`に更新。全88件pass）*

*過去の更新: 2026-07-10（アーキテクチャ監査に基づくノーリスク整理: `utils/common_utils.py` の
`handle_error()` を削除（Model層のstreamlit依存を解消、呼び出し元の `app.py` へインライン化）。
`app.py` の未使用import（`extraction_config`）・死んだコメント行も削除）*

*過去の更新: 2026-07-10（`update_parent_child_master()` で、既存台帳のエンティティ数カラムが
...
*過去の更新: 2026-07-08（Step4「オプション設定」に**色だけが異なる図形は変更なし扱いにする**
（`ignore_color_only_changes`）を追加。実データ `EE2505-633-43E_vs_43D` で、座標・形状
（LINE の start/end、CIRCLE の center/radius）が完全一致するのに `color`（多くは
`lineweight` も）だけが異なるエンティティが334件、DELETED＋ADDED として検出される
現象を確認。パターン化された分布（`lineweight 25→50・color 7→4` が231件等）から
改訂マーキング等の意図的な色分け変更と判断し、無条件UNCHANGED化ではなくユーザーに
意図を確認したうえでオプトインの選択肢として実装した。`SignatureGenerator` に
`ignore_color` を追加し、署名の重要属性リストから `color` のみを除外可能にする
（`height`/`radius`/`start_angle`/`end_angle` は幾何形状属性のため対象外。`lineweight`
は元々署名に含まれていない）。`compare_dxf_files_and_generate_dxf()` /
`count_entities_in_dxf_file()` の両方に同名引数を追加し、完全新規図面の Total Entities
算出とも定義を揃えた。回帰テスト5件を `tests/unit/test_compare_dxf.py` に追加。
全86テスト（新規5件）全パス）*

*過去の更新: 2026-07-08（dev-workflow スキル改訂（選択肢の組み合わせ表の作成を Phase 2-1 に
必須化）を受け、直近の3件の修正・機能追加で未検証だった組み合わせを洗い出して検証。
(1) `ignore_moved_labels` × 未変更ラベル先頭文字列フィルタ（`filter_unchanged_by_prefix`）・
機器符号妥当性チェック（`validate_ref_designators`）: 検証の結果、既存実装は正しく動作
しており不具合なし。回帰テスト `test_ignore_moved_labels_combined_with_prefix_filter_and_ref_designator_check`
を追加してロック。
(2) 図面管理台帳アップロード × Step1の3方式（Type A/B の RevUp・流用、Type C の
ペアリスト）: `update_parent_child_master()` は pairs のスキーマのみに依存し mode
分岐を持たないため構造的に安全なはずだったが、実際にアップロード経由で読み込んだ
DataFrame に対して新規追加・既存更新の両方を確認。不具合なし。回帰テスト
`test_uploaded_master_merges_correctly_across_all_pairing_modes` を追加してロック。
(3) diff_labels.xlsx のシート ABC順ソート × unchanged_labels.xlsx: 実際に順序不一致
（diff_labelsはABC順、unchanged_labelsは処理順のまま）を確認。ユーザー確認のうえ
unchanged_labels.xlsx 側も `sheet_name` で直接ソートするよう対応（9.4節参照）。
回帰テストを `test_diff_labels_summary_and_sheets_sorted_alphabetically_by_drawing_number`
に統合。全81テスト（新規5件）全パス)*

*過去の更新: 2026-07-08（Step0「既存の図面管理台帳のアップロード」で、このツール自身が
...
ADDED とする契約（標準的な diff の慣習: 旧→新）のため、実際には新旧が完全に逆転していた
（実データ `EE4144-613-49D_vs_EE4144-613-49C` で、ADDED レイヤーに旧図面自身のテキスト
`EE4144-613-49C` が、DELETED レイヤーに新図面自身のテキスト `EE4144-613-49D` と新規追加
された改訂メモが混入することで発覚）。呼び出し順序を `(source_file_path=旧, main_file_path=新)`
に修正し、8.6節の関数ドキュメント自体に含まれていた誤った説明（file_a=新・「Aにのみ存在＝
追加」）も訂正した（この誤記が呼び出し順序の誤りの元凶だった可能性が高い）。図面管理台帳の
Deleted/Added Entities 列もこの同じ辞書を使うため合わせて修正される。回帰テストを
`tests/unit/test_compare_dxf.py`（`test_file_a_only_is_deleted_file_b_only_is_added`）と
`tests/unit/test_diff_export.py`（`create_diff_zip()` のエンドツーエンド検証）に追加。
diff_labels.xlsx（`compute_label_differences(new_file, old_file, ...)`）は元々正しい順序で
呼ばれており影響なし）*

*過去の更新: 2026-07-08（Step4「オプション設定」に**移動しただけのラベルを差分から除外**
（`ignore_moved_labels`）を追加。回路ブロックがまるごと別座標に移動すると、
`find_label_change_pairs()` は座標単位でしか突き合わせないため移動元「削除」・移動先
「追加」として大量に検出される（結線・ラベル自体は変わっていないのに diff_labels.xlsx が
偽差分を報告する）。`utils/label_diff.py` に `reclassify_moved_labels()` を新設し、
Old Label のみ（削除）／New Label のみ（追加）の行を対象に、同一ラベル文字列の削除件数・
追加件数が一致する分を座標を無視して「変更なし」に振り替える。同一座標での名称変更（Old/New
両方が存在する行）は対象外。「☆」を含むラベル（注記用）は件数が一致していても常に変更候補の
まま残す。デフォルトは無効（オプトイン。既存呼び出し元の挙動は変えない）。UIには他の
オプションと同様に `st.checkbox` の `help=` でリスク（座標を見ず件数だけで判定するため、
たまたま同数の無関係な削除・追加も「移動」とみなされる）を明記。回帰テスト
`tests/unit/test_label_diff.py` を新設（6件 全パス。実データ
EE6606-613-49A_vs_EE4144-613-49D で変更行146→42・未変更386→438を確認）。

*過去の更新: 2026-07-08（差分DXF（`compare_dxf.py`）の2件の不具合修正＋座標マージン既定値変更。
