...
auto モード用のペアリング関数。流用判定と RevUp 判定を**独立した2パス**で実行し、両方のペアを出力する（`create_pairs_from_single_pool` と同じ方針。ただし RevUp/流用とも比較元は流用元グループ、比較先は流用先グループに限定される）。

**ロジック**:
1. **RevUp パス**: `create_revup_pairs(source_files_dict, dest_files_dict)` を実行し、`status='complete'`, `relation='RevUp'` のペアを生成。キー `(比較先, 比較元)` を記録する。
...
`create_revup_pairs()` 自体は流用元×流用先で 1:1 の RevUp ペアを生成する（同一ベースに複数版がある場合は連続ペア）。呼び出し側（`create_pair_list` / `create_pairs_from_single_pool`）は **RevUp 判定と流用判定を独立して実行**し、完全に同一の（比較先, 比較元）ペアのみ重複排除して両方を出力する（RevUp を優先消費して流用判定から除外する旧仕様は 2026-06 に廃止）。
