...
Step 1 で「新規作成」または「既存ファイルのアップロード」を選択して台帳を設定する。処理完了後、更新したファイルをダウンロードZIPに含める。
...
| 選択肢 | 動作 |
|---|---|
| **新規作成**（デフォルト） | ファイル名を入力欄で指定し（デフォルト: `図面管理台帳`）、空の台帳を自動初期化する。`.xlsx` は自動付与。 |
...
- Step 2-2: 比較元・比較先のすべてのDXFファイルをまとめてアップロードし「ファイルを読み込む」をクリック
...
| `new_master_filename_input` | str | 新規作成時の台帳ファイル名（拡張子なし、デフォルト: `図面管理台帳`） |
