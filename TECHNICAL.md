...
| 完全新規図面（流用元図番なし） | `no_source_defined` のうち、流用元・流用先の両グループに同一図番が存在しない図面 | 旧名「流用元図番の記載がない図面（比較対象外）」。`関係`列は固定で「完全新規図面」、`ステータス`列は固定で「流用元図番なし」（⚠️マークなし） |
| 変更していない図面（流用元と流用先とで共通） | 流用元グループと流用先グループに同一図番が共通して存在するもの | **Type A（all_in_one）では表示しない**。Type B は `source_files_dict.keys() & dest_files_dict.keys()`、Type C は `status='identical'` のペアから算出 |

`no_source_defined` のうち、上記の「変更していない図面」の対象図番に該当するものは「完全新規図面」セクションから除外され、「変更していない図面」側にのみ表示される。一方、`missing_source`（流用元側が別の図番を実際に参照している場合）はこの除外ロジックの対象外であり、同一図番が「流用元図番の図面がない図面」と「変更していない図面」の両方に現れることがある（DXF内に埋め込まれた流用元参照と、ファイルの重複アップロードは独立した事実のため）。
...
| `new_master_module_input` | str | 新規作成時のモジュール入力値（例: `1111`、空可） |
...
- **変更していない図面（流用元と流用先とで共通）（2026-06 追加）**: `mode = st.session_state.step1_mode` に応じて対象図番集合 `unchanged_drawings` を算出する。
  - Type A（`all_in_one`）: 流用元・流用先の区別がないため、このセクション自体を表示しない（`unchanged_drawings = set()`）
  - Type B（`auto`）: `source_files_dict.keys() & dest_files_dict.keys()`（流用元・流用先の両グループに同一図番のファイルがあるもの）
  - Type C（`pair_list`）: `status='identical'` のペアの `main_drawing` 集合
  - 表示は `unchanged_drawings` の単純な集合であり、同じ図番が `missing_source`（DXF内に埋め込まれた別の流用元参照が未アップロードの場合）にも独立して現れることがある（重複表示は仕様。アップロードの重複と DXF 内の参照は別の事実のため）。
...
*最終更新: 2026-06-24（用語統一: 「比較元/比較先」→「流用元/流用先」（ペアリストの旧カラム名は後方互換）。Step 1 を「既存アップロード/新規作成（指番・モジュール・サイドから台帳ファイル名を自動生成）/作成せず」の3択に再設計。Step 3 のペアリスト表示を全セクション「：N件」表記に統一し、「完全新規図面」「変更していない図面（流用元と流用先とで共通）」セクションを追加。Summaryシートの図面統計・総図形数ラベルをペアリング方式（Type A/B/C）別に変更）*
