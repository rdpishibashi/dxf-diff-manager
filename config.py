"""
DXF Diff Manager - 設定ファイル

このファイルにはアプリケーション全体で使用される設定値が定義されています。
"""


class UIConfig:
    """UI関連の設定"""

    # ファイルアップロード設定
    MASTER_FILE_TYPES = ["xlsx"]   # 図面管理台帳ファイルの拡張子
    DXF_FILE_TYPES = ["dxf"]       # DXFファイルの拡張子

    # メッセージ設定
    TITLE = "DXF Diff Manager - 図面差分管理ツール"
    SUBTITLE = "アップロードしたDXFファイルからペアを抽出して、差分DXF図面とラベル差分リストを出力します。図面管理台帳も更新できます。"


class DiffConfig:
    """差分比較関連の設定"""

    # デフォルト値
    DEFAULT_TOLERANCE = 0.01       # 座標許容誤差

    # DXFレイヤー色設定（AutoCADカラーインデックス）
    DEFAULT_DELETED_COLOR = 6      # 削除エンティティ（マゼンタ）
    DEFAULT_ADDED_COLOR = 4        # 追加エンティティ（シアン）
    DEFAULT_UNCHANGED_COLOR = 7    # 変更なしエンティティ（白/黒）

    # 色の選択肢（label, value）
    COLOR_OPTIONS = [
        (1, "1 - 赤"),
        (2, "2 - 黄"),
        (3, "3 - 緑"),
        (4, "4 - シアン"),
        (5, "5 - 青"),
        (6, "6 - マゼンタ"),
        (7, "7 - 白/黒"),
    ]

    # ZIPファイル名
    OUTPUT_ZIP_FILENAME = "dxf_diff_results.zip"
    MASTER_FILENAME = "Parent-Child_list.xlsx"


class ExtractionConfig:
    """DXF抽出関連の設定"""

    # 図番抽出設定
    # 両フォーマット対応: XX0000-000-00X（長）、XX0000-000X（短）
    DRAWING_NUMBER_PATTERN = r'[A-Z]{2}\d{4}-\d{3}(?:-\d{2})?[A-Z]'  # 図番パターン

    # 距離設定（DXF単位）
    SOURCE_LABEL_PROXIMITY = 80     # 流用元図番ラベルからの検出距離
    DWG_NO_LABEL_PROXIMITY = 80     # DWG No.ラベルからの検出距離
    TITLE_PROXIMITY_X = 80          # TITLEラベルからの横方向検出距離

    # RevUpペア設定
    RIGHTMOST_DRAWING_TOLERANCE = 100.0  # 右端図面判定の許容範囲


class HelpText:
    """ヘルプテキスト"""

    USAGE_STEPS = [
        "このツールは、アップロードしたDXFファイルからペアを作成し、",
        "ペアごとに差分を比較して差分のDXFファイルとラベルリストを出力します。",
        "",
        "**使用手順：**",
        "1. ペアリングの方法を選択します。",
        "2. 図面管理台帳を「新規作成」または「既存ファイルのアップロード」で設定します（処理後にZIPへ含まれます）",
        "   新規作成の際にはファイル名を対応する指番にしてください。",
        "3. 選択したペアリング方法により、次のいずれかでDXFファイルをアップロードします：",
        "   (Type A) DXFファイルを一括してアップロードします（流用元図番を自動的に抽出します）",
        "   (Type B) 流用元（旧）と流用先（新）のDXFファイルを別々にアップロードします",
        "   (Type C) 比較する図番のペアリストと全DXFファイルをアップロードします",
        "4. 「ファイルを読み込む」ボタンで、データ解析と必要な図番抽出を行います",
        "5. 「図面ペア・リスト作成」ボタンでペアを生成します",
        "5. 「差分抽出開始」ボタンをクリックして処理を実行します",
        "6. 完全なペアのみが処理され、ZIPファイルで一括ダウンロードできます",
        "7. ZIPには、差分DXFファイル、変更されたラベルリスト、変更していないラベルで指定の先頭文字列のラベルリスト、更新された図面管理台帳が含まれます",
        "",
        "**出力DXFファイルの内容：**",
        "- ADDED (デフォルト色: シアン): 新図面にのみ存在する要素（追加された図形要素）",
        "- DELETED (デフォルト色: マゼンタ): 旧図面にのみ存在する要素（削除された図形要素）",
        "- UNCHANGED (デフォルト色: 白/黒): 両方の図面に存在し変更がない図形要素",
        "",
        "**注意事項：**",
        "- 流用元・流用先ペアリングでは両者間でのみの比較です（流用元内、流用先内ではペアリングしません）",
        "- 図面管理台帳には、有効な比較図番ペアのみが追加されます"
    ]


# 設定クラスのインスタンスを作成（簡単にアクセスできるように）
ui_config = UIConfig()
diff_config = DiffConfig()
extraction_config = ExtractionConfig()
help_text = HelpText()
