...
- **ラベル比較（自動）**
  - `diff_labels.xlsx` に各図面ペアのラベル変更候補を新図番ごとに出力（列名は `Old: {旧図番}` / `New: {新図番}`）
  - `unchanged_labels.xlsx` に指定プレフィックスに一致する未変更ラベル（Label / Count / Coordinate X / Coordinate Y）を新図番ごとに出力
  - プレフィックスは UI から編集でき、初期値は `prefix_config.txt` から読み込まれる
...
4. **ステップ3: 差分比較**
   - オプション設定（座標許容誤差、レイヤー色、未変更ラベル抽出プレフィックス）を調整
   - 「差分比較を開始」ボタンをクリック
   - 完全なペアのみが処理される
   - 各ペアのエンティティ数に加えて「変更ラベル数」「未変更抽出ラベル数」を表示
   - 処理完了後、ZIPファイルをダウンロード
   - ZIPには差分DXF、`diff_labels.xlsx`、`unchanged_labels.xlsx`、更新済みマスター（アップロードした場合）が含まれる
...
- 差分DXFファイル（各ペアごと）
- `diff_labels.xlsx`（図番（新）ごとのシートにラベル変更候補を掲載。列名は `Old: {旧図番}` / `New: {新図番}`）
- `unchanged_labels.xlsx`（図番（新）ごとのシートにプレフィックス一致の未変更ラベルを掲載。Label / Count / Coordinate X / Coordinate Y）
- 親子関係台帳ファイル（アップロードした場合）

DXFファイルには以下の3つのレイヤーで差分が表示されます：
...
### 座標許容誤差
- デフォルト: 0.01
- 微小な座標のずれを許容する誤差値（DXF差分およびラベル比較の双方で使用）
...
### 未変更ラベル抽出プレフィックス
- `prefix_config.txt` の内容を初期値として読み込み
- UIのテキストエリアで1行につき1件のプレフィックスを編集可能
- 指定されたプレフィックスで始まる未変更ラベルのみを `unchanged_labels.xlsx` に出力
