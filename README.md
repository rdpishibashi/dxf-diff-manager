...
### 図番が正しく抽出されない
- 図面番号が標準パターン（例: `DE5313-008-02B`）に従っているか確認
- ファイル名を図番として使用することもできます

### 流用元図面が見つからない
- 流用元図番が図面内に記載されているか確認
- 「追加アップロード」で流用元図面を追加
...
このアプリはStreamlit Cloudで実行できます：

1. GitHubリポジトリにプッシュ
2. Streamlit Cloudでアプリを作成
3. `DXF-diff-manager/app.py` を指定
4. `requirements.txt` が自動的に認識される

## ライセンス

ULVAC Electric Design Management Tools

## 更新履歴

- 2025-11-25 (v1.3): プロジェクト名称変更
  - プロジェクト名を「DXF Diversion Diff」から「DXF Diff Manager」に変更
  - ディレクトリ名を `DXF-diversion-diff` から `DXF-diff-manager` に変更
  - 親子関係マスター管理機能を含む包括的な名称に

- 2025-11-25 (v1.2): マスター管理機能の最終調整
  - Date カラムに実行日時を自動記入するよう修正
  - ボタンの枠色も青色に変更（背景色と枠色を統一）
  - Streamlit非推奨警告を修正（use_container_width → width='stretch'）

- 2025-11-25 (v1.1): UI改善とマスター管理機能の調整
  - ボタンの背景色を青色に変更
  - マスター読込ボタンを削除（ファイルアップロード時に自動処理）
  - Function カラムへの自動書き込みを停止（既存値を保護）

- 2025-11-25 (v1.0): 初版リリース
  - 自動ペアリング機能
  - 差分比較処理
  - 親子関係マスター管理機能の追加
  - 改善された図面番号抽出ロジック（ラベル近接性検出）
